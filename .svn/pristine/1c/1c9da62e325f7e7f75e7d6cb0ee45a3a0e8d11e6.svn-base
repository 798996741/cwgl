...
<div>
    ...
    <div id="mask" class="mask">
        <div id="darkmask" class="darkmask">
            ...
            <!-- form start -->
            <div class="mask-main">
                <form action="client/insertSellp.do" name="form_add" id="form_add" method="post">

                    <%--<c:forEach items="${sellp}" var="sellp" begin="0" end="0">--%>
                    <input type="hidden" name="cliid" id="cliid" value="${pd.cliid}"/>
                    <%--</c:forEach>--%>
                    <div class="between-center">
                        <div class="between-center padding-8px">
              <span class="mask-size flex-row-between">
                <font style="color: red;">*</font>物品名称:
              </span>
                            <input type="text" name="itemname" id="itemname" autoComplete="off" maxlength="32"
                                   placeholder="这里输入物品名称"
                                   title="物品名称" class="mask-input width-250px"/>
                        </div>
                        <div class="between-center padding-8px">
              <span class="mask-size flex-row-between">
                <font style="color: red;">*</font>单&nbsp;&nbsp;&nbsp;&nbsp;价
              </span>
                            <input type="text" name="price" id="price" autoComplete="off" placeholder="这里输入单价"
                                   maxlength="64"
                                   title="单价" title="账号" class="mask-input width-250px"/>
                        </div>
                    </div>
                    ...
                    <div class="between-center">
                        <div class="between-center padding-8px">
                            ...
                            <input type="text" name="remark" id="remark" autoComplete="off" maxlength="32"
                                   placeholder="这里输入备注"
                    ...
                    <div class="seat-middle-r" style="width: 100%;height: 270px;">
                        <form name="form_add2" id="form_add2">
                            <table border="0" cellspacing="0" cellpadding="0">
                                ...
                                <c:choose>
                                    <c:when test="${not empty sellp}">
                                        <%--<c:if test="${QX.cha == 1 }">--%>
                                        <c:forEach items="${sellp}" var="sellp" varStatus="vs">
                                            <input type="hidden" name="sellpid" id="sellpid" value="${sellp.sellpid}"/>
                                            <tr>

                                                <td class="center">${vs.count }</td>
                                                <td class="center">${sellp.itemname }</td>
                                                <td class="center">${sellp.price }</td>
                                                <td class="center">${sellp.username}</td>
                                                <td class="center">${sellp.cztime}</td>
                                                <td class="center">${sellp.remark }</td>
                                                ...
                                                <td>
                                                    <div>
                                                        ...
                                                        <i><img src="static/img/home/delete.png"
                                                                onclick="del('${sellp.sellpid}');"
...
<script type="text/javascript">
    ...
    function editSellp(client_id) {
        ...
        modals.openWin({
            ...
            title: '新增',
            width: '900px',
            url: '<%=basePath%>client/findSellpById.do?cliid=' + client_id
            /*, hideFunc:function(){
                modals.info("hide me");
            },
            showFunc:function(){
    ...
    function del(sellpid) {
        if (confirm("确定要删除吗?")) {

            var url = "<%=basePath%>client/deleteSellpById.do?sellpid=" + sellpid;
            $.ajax({
                type: 'post',
                data: {sellpid: sellpid},
                url: url,
                cache: false,
                dataType: 'text',
                success: function (data) {
